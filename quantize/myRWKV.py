...
class RWKV(MyModule):
    ...
    def att_one(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
        ...
        r = torch.sigmoid(self._trigger_gptq(rx, weight=rw[0], name=rw[1]))
        k = self._trigger_gptq(kx, weight=kw[0], name=kw[1]).float()
        v = self._trigger_gptq(vx, weight=vw[0], name=vw[1]).float()
        ...
        # out = (r * wkv) @ ow
        out = self._trigger_gptq(r * wkv, weight=ow[0], name=ow[1])
    ...
    def _trigger_gptq(self, x, weight, name):
        
        # GPTQ
        if name in self.w_quant.keys():
            w_quant = self.w_quant[name]
            # Only work on CUDA because of the use of fp16 (not available on CPU)
            if w_quant.bits in [2, 4, 8]:    
                w_quant.scales = w_quant.scales.to(x.device)
                out_shape = x.shape[:-1] + (w_quant.outfeatures, )
                x = x.reshape(-1,x.shape[-1])
                
                zeros = torch.bitwise_right_shift(torch.unsqueeze(w_quant.qzeros, 2).expand(-1, -1, 32 // w_quant.bits), w_quant.wf.unsqueeze(0)).to(x.device, torch.int16 if w_quant.bits == 8 else torch.int8)
    ...
    def forward(self, tokens, state, full_output=False):
        with torch.no_grad():
            ...
            for i in range(args.n_layer):
                ...
                kw = w[f'{att}key.weight'], f'{att}key.weight'
            ...
            if w['head.weight'].dtype != torch.uint8:
                x = x @ w['head.weight']
                # FIXME: WTF check load model and see why
                # (Pdb++) w_quant.scales.shape
                # torch.Size([1, 50277])
                # (Pdb++) zeros.shape
                # torch.Size([1, 12568, 4])
                # (Pdb++) 12568 * 4
                # 50272
                # x = self._trigger_gptq(x, w["head.weight"], "head.weight")
            else:
