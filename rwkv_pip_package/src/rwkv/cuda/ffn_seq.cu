...
__global__ void _ffn_mix(const half *xx, const half *sx, const half *k_mix,
...
void ffn_mix(const half *xx, const half *sx, const half *k_mix,
  ...
  _ffn_mix<<<inner_size / BLOCK_SIZE, BLOCK_SIZE>>>(
...
struct InplaceReLUAndSquare {
  __device__ __forceinline__ half operator()(int i) const {
    half t = __hmax(__float2half(0.0), ptr[i]);
    ptr[i] = __hmul(t, t);
  }
  half *ptr;
};
...
Tensor ffn_seq(Tensor x, Tensor sx, Tensor ln_w, Tensor ln_b, Tensor k_mix,
  ...
  ffn_mix(data_ptr<half>(xx), data_ptr<half>(sx), data_ptr<half>(k_mix),
          data_ptr<half>(r_mix), xx.size(0), xx.size(1), kx, rx);

  gemm_fp16_cublas(rx, rw.data_ptr(), r, x.size(0), rw.size(1), x.size(1), false);
  element_wise(InplaceSigmoid{r}, x.size(0) * rw.size(1));
  gemm_fp16_cublas(kx, kw.data_ptr(), vx, x.size(0), kw.size(1), x.size(1), false);
